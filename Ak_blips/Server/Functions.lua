...
function RefreshBlips(src, remover)
    ...
    "SELECT * FROM ak-blips WHERE identifier = @steam OR universal = 1",
    {['@steam'] = iden})

    if remover then 
        TriggerClientEvent('ak-blipsCreator:ByeBye', -1, result, remover)
    else 
        TriggerClientEvent('ak-blipsCreator:LoadUniversalBlips', src, result, remover)
    end
...
function GetBlipsToDelete(src, admin)
    ...
    local iden = GetIdentifier(src, Config.Identifier)
    local result = Query(Config.Db, 'fetchAll',
    "SELECT * FROM ak-blips WHERE identifier = @steam",
    {['@steam'] = iden})

    local result2 = Query(Config.Db, 'fetchAll',
    "SELECT * FROM ak-blips WHERE universal = 1")

    if admin == 'yes' then 
        TriggerClientEvent('ak-blipsCreator:BlipsABorrar', src, result2)
    else
        TriggerClientEvent('ak-blipsCreator:BlipsABorrar', src, result)
    end
